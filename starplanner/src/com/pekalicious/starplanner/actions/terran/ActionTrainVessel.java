...
public class ActionTrainVessel extends StarAction implements Serializable {
	private static final long serialVersionUID = -82342333641738L;
	...
	public void setupConditions() {
		...
		this.effects.setProperty(WSKey.T_SCIENCE_VESSEL, new WorldStateValue<Boolean>(true));
		this.cost = 10.0f;
		this.precedence = 1;
	}
	...
	public void activateAction(Agent aiManager, WorldState state) {
		int count = 1;
		count -= Game.getInstance().self().completedUnitCount(UnitType.TERRAN_SCIENCE_VESSEL);

		if (count > 0) {
			StarBlackboard bb = (StarBlackboard)((StarPlanner)aiManager).getBlackBoard(); 
			order = bb.addToTrainingQueue(UnitUtils.Type.TERRAN_SCIENCE_VESSEL, count);
		}else{
