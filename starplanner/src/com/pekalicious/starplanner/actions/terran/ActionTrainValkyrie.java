...
public class ActionTrainValkyrie extends StarAction implements Serializable {
	private static final long serialVersionUID = -82342335255638L;
	...
	public void setupConditions() {
		...
		this.preconditions.setProperty(WSKey.T_CONTROL_TOWER, new WorldStateValue<Boolean>(true));
		
		
		this.effects = new WorldState();
		this.effects.setProperty(WSKey.T_VALKYRIE, new WorldStateValue<Boolean>(true));
		this.cost = 10.0f;
		this.precedence = 1;
	}
	...
	public void activateAction(Agent aiManager, WorldState state) {
		int count = 1;
		count -= Game.getInstance().self().completedUnitCount(UnitType.TERRAN_VALKYRIE);

		if (count > 0) {
			StarBlackboard bb = (StarBlackboard)((StarPlanner)aiManager).getBlackBoard(); 
			order = bb.addToTrainingQueue(UnitUtils.Type.TERRAN_VALKYRIE, count);
		}else{
