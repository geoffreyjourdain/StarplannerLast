...
public class ActionBuildStarport extends StarAction {
	private static final long serialVersionUID = 164564645919635291L;
	...
	public void setupConditions() {
		...
		this.effects.setProperty(WSKey.T_STARPORT, new WorldStateValue<Boolean>(true));
	...
	public void activateAction(Agent aiManager, WorldState state) {
		int count = Game.getInstance().self().completedUnitCount(UnitType.TERRAN_STARPORT);

		if (count == 0) {
			StarBlackboard bb = (StarBlackboard)((StarPlanner)aiManager).getBlackBoard(); 
			order = bb.addToBuildQueue(UnitUtils.Type.TERRAN_STARPORT, count);
		}else{
	...
	public void applyContextEffect(Agent agent, WorldState state, WorldState worldState) {
		state.setProperty(WSKey.T_STARPORT, new WorldStateValue<Boolean>(true));
	}
