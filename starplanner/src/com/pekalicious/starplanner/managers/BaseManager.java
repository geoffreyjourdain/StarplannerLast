...
public class BaseManager {
	...
	public BaseManager(BaseLocation location, StarBlackboard blackboard) {
		...
		this.maxMineralGatherers = this.mineralPatches.size() * 3;
	...
	public void update() {
		...
		for (Unit unit : Game.getInstance().unitsOnTile(x, y)) {
			if (unit.getType().equals(UnitType.TERRAN_FACTORY)) {
				factory = unit;
				Logger.Debug("BASEMANAGER:\tL'usine a été trouvée !!\n", 1);
				break;
			}else {
				Logger.Debug("BASEMANAGER:\tVous n'avez pas encore d'usine !!\n", 1);
			}
		}
