...
import com.pekalicious.starplanner.util.UnitUtils; // ajout
...
public class BuilderManager {
	...
	private List<Unit> buildings; //ajout
	private BuildOrder orderb;
	public BuilderManager(Game game, StarBlackboard blackBoard, List<BaseManager> baseManagers) {
		...
		this.buildings = new ArrayList<Unit>(); //ajout
	}
	...
	public void update() {
		...
		for (BuildOrder order : blackBoard.buildQueue) {
			...
			if (order.factory == null) {
				Unit factory = UnitUtils.getIdleFactory(game, blackBoard);
				if (factory == null) {
					//Logger.Debug("!!! pas d'usine\n", 1);
				}else{
					Logger.Debug("prise pour creation !!! \n", 1);
					order.factory = factory;
				}
			}
			
			if (order.unitType == UnitUtils.Type.TERRAN_MACHINE_SHOP) {
				Logger.Debug("Je veux un machine shop  !!! \n", 1);
				
				
				
				
				//boolean buildable = true;
                for (int x = order.factory.getTilePosition().x() + 4; x < order.factory.getTilePosition().x() + 6; x++) {
                    for (int y = order.factory.getTilePosition().y() + 1; y < order.factory.getTilePosition().y() + 3; y++) {
                        
                    }
                }
                /*
                order.factory.freeTiles(order.factory.getTilePosition(), 4, 3);
                order.factory.getTilePosition() = order.factory.getBuildLocationNear(order.factory.tilePosition, 
            ...
            placer.reserveTiles(building.tilePosition.opAdd(new TilePosition(4, 1)), 2, 2);
				*/
                ...
                //Logger.Debug("x et y valent : " + order.factory.getTilePosition().x() + " " + order.factory.getTilePosition().y() + ")\n", 1);
				...
				/*if (order.buildUnit == null) {
					Logger.Debug("J'ai compris :) \n", 1);
					order.buildUnit = order.factory.getAddon();
					Logger.Debug("fait ou pas ?? \n", 1);
				}*/
			//}
